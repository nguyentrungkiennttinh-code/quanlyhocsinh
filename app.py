...
# Tên worksheet chính xác bạn đã yêu cầu
...
def load_data():
    try:
        # Đọc dữ liệu từ worksheet cụ thể
        df = conn.read(worksheet=SHEET_NAME, ttl=0)
        return df.dropna(how="all")
    except Exception:
        # Nếu Sheets trống hoặc chưa có bảng, tạo khung dữ liệu mặc định
...
if menu == "Học sinh đăng ký":
    ...
    with st.form("form_dang_ky", clear_on_submit=True):
        ...
        if st.form_submit_button("Gửi đơn đăng ký"):
            if not ten:
                st.error("Vui lòng nhập đầy đủ họ tên học sinh!")
            else:
                df_existing = load_data()
                # Tự động tạo ID đơn tiếp theo
                new_id = int(pd.to_numeric(df_existing["Mã Đơn"], errors='coerce').max() + 1) if not df_existing.empty else 1
                
                new_row = pd.DataFrame([{
                    "Mã Đơn": new_id, "Họ Tên": ten, "Lớp": lop, "Loại Hình": loai_hinh, 
                    "Chi Tiết Người Đón": chi_tiet, "CCCD Người Đón": cccd, 
                ...
                st.cache_data.clear()
                st.success(f"✅ Gửi đơn thành công! Mã số đơn của bạn là: {new_id}")
...
# --- 2. GIAO DIỆN GIÁO VIÊN ---
    ...
    if pw == PASS_GVCN:
        lop_ql = st.selectbox("Chọn lớp bạn quản lý:", ["10A1", "10A2", "10A3", "10A4", "10A5", "10A6", "11A1", "11A2", "11A3", "11A4", "11A5", "11A6", "12A1", "12A2", "12A3", "12A4", "12A5"])
        df = load_data()
        # Lọc các đơn đang chờ của đúng lớp
        ...
        if st.button("Xác nhận Đơn"):
            if not df_show.empty and id_gv in df_show["Mã Đơn"].values:
                ...
                st.warning("Mã đơn không hợp lệ hoặc không thuộc lớp của bạn.")
...
# --- 3. GIAO DIỆN QUẢN LÝ ---
    ...
    if pw_ad == PASS_QUANLY:
        df_all = load_data()
        # Lọc các đơn đã được GVCN xác nhận
        df_admin = df_all[(df_all["GVCN Duyệt"] == "Đã xác nhận") & (df_all["Quản lý Duyệt"] == "Chờ duyệt")]
        st.dataframe(df_admin, use_container_width=True)
        
        id_ql = st.number_input("Nhập Mã đơn để cấp phép:", step=1, min_value=0)
        if st.button("🚀 CẤP PHÉP CHÍNH THỨC"):
            if not df_admin.empty and id_ql in df_admin["Mã Đơn"].values:
                ...
                st.cache_data.clear()
                st.success(f"Đã phê duyệt đơn số {id_ql}")
                st.rerun()
