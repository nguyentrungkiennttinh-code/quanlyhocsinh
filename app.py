...
# Khởi tạo cơ sở dữ liệu tạm thời trong bộ nhớ
if 'db_requests' not in st.session_state:
    st.session_state.db_requests = pd.DataFrame(columns=[
        "Mã Đơn", "Họ Tên", "Lớp", "Loại Hình", "Chi Tiết Người Đón", "CCCD Người Đón", "GVCN Duyệt", "Quản lý Duyệt", "Trạng Thái"
    ])
    ...
    with st.form("form_dang_ky"):
        ...
        if loai_hinh == "Về nhà cuối tuần":
            ...
            if nguoi_don != "Tự đi xe khách về":
                ...
                with c2: cccd = st.text_input("Số CCCD người đón (Nếu có):")
...
# 3. GIAO DIỆN QUẢN LÝ / BGH (CÓ NÚT TẢI VỀ)
    ...
    if pw_ad == PASS_QUANLY:
        ...
        # 2. Khu vực Xuất dữ liệu (Đây là nút bạn cần)
        st.markdown("---")
        st.subheader("📥 Xuất dữ liệu báo cáo")
        if not df_all.empty:
            # Chuyển đổi sang CSV hỗ trợ tiếng Việt (utf-8-sig)
            ...
            st.info("Chưa có dữ liệu để tải về.")
    elif pw_ad != "":
